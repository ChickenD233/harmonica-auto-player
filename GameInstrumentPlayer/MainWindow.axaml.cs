...
using System.Text;
using GameInstrumentPlayer.Engine;
using GameInstrumentPlayer.Input;
using GameInstrumentPlayer.Midi;
using GameInstrumentPlayer.Persist;
...
public partial class MainWindow : Window
{
    ...
    // —— 乐器方案 ——
    private List<InstrumentProfile> _profiles = new();
    private InstrumentProfile _profile = BuiltInProfiles.Default();
    private bool _profileReady;        // 载入方案清单期间不要触发保存
    ...
    public MainWindow()
    {
        ...
        // 输入兼容档位：决定修饰键与琴键之间的物理时间余量
        TimingCombo.ItemsSource = InputTiming.Names;
        TimingCombo.SelectedIndex = 1;          // 方案默认
        ...
        InsertLog("欢迎使用 MIDI 转游戏乐器演奏器");
        InsertLog($"当前乐器方案：{_profile.Name}（音域 {_profile.RangeLabel}）。想适配别的游戏，点「编辑键位…」。");
        InsertLog("用法：选好方案 → 打开 MIDI → 点一行作为主旋律 → 按 F6，倒计时内切到游戏并打开乐器。");
        ...
        InstallDevSnapshot(this);   // 【开发用，可删】设了 GIP_UI_SNAPSHOT 才生效，见 DevUISnapshot.cs
    }

    // ================= 乐器方案 =================

    /// <summary>重新读取方案清单并选中指定方案（找不到就用第一个）。</summary>
    private void ReloadProfiles(string? wantId)
    {
        _profileReady = false;
        var (all, _, _) = ProfileStore.LoadAll();
        _profiles = all;
        if (_profiles.Count == 0) _profiles.Add(BuiltInProfiles.Default());
        ...
        {
            int found = _profiles.FindIndex(p => string.Equals(p.Id, wantId, StringComparison.OrdinalIgnoreCase));
            if (found >= 0) idx = found;
        }
        ProfileCombo.SelectedIndex = idx;
        _profileReady = true;
        ApplyProfile(_profiles[idx]);
    }

    /// <summary>把方案写进映射器与界面说明。</summary>
    private void ApplyProfile(InstrumentProfile profile)
    {
        _profile = profile;
        NoteMapper.Profile = profile;

        var user = profile.UserDefined ? "（自建）" : "（内置）";
        TxtProfileInfo.Text =
            $"{user} {profile.Description}\n" +
            $"音域 {profile.RangeLabel}；升半音：{SharpModes.Label(profile.SharpMode)}；" +
            $"输出：{OutputMethods.Label(profile.OutputMethod)}；" +
            $"按住 {profile.HoldMs}ms / 重触发 {profile.RetriggerMs}ms / 修饰键提前 {profile.ModLeadMs}ms";

        BtnEditProfile.Content = profile.UserDefined ? "编辑键位…" : "复制并编辑…";

        if (_profileReady)
        {
            _cfg.ProfileId = profile.Id;
            ScheduleSave();
            RefreshPreview();
            InsertLog($"已切换乐器方案：{profile.Name}{user}");
        }
    }
    ...
    private void Profile_Changed(object? sender, SelectionChangedEventArgs e)
    {
        ...
        ApplyProfile(_profiles[i]);
    }

    /// <summary>编辑当前方案。内置方案先复制一份，改完另存，保证内置方案可以还原。</summary>
    private async void BtnEditProfile_Click(object? sender, RoutedEventArgs e)
    {
        var draft = _profile.Clone();
        draft.Id = "";
        if (!_profile.UserDefined)
        {
            draft.Name = _profile.Name + "（我的）";
            draft.UserDefined = true;
        }

        var win = new ProfileEditorWindow(draft, _profile.UserDefined);
        ...
        {
            // 在编辑器里删掉了这个方案
            InsertLog($"已删除方案：{_profile.Name}");
            ReloadProfiles(null);
            return;
        }

        if (!ProfileStore.Save(win.Result, out string err))
        {
            InsertLog($"方案保存失败：{err}");
            return;
        }
        InsertLog($"已保存方案：{win.Result.Name}（{ProfileStore.PathOf(win.Result.Id)}）");
        ReloadProfiles(win.Result.Id);
    }

    /// <summary>新建：复制当前方案，改完另存。</summary>
    private async void BtnNewProfile_Click(object? sender, RoutedEventArgs e)
    {
        var draft = _profile.Clone();
        draft.Id = "";
        draft.Name = _profile.Name + " 副本";
        draft.UserDefined = true;

        var win = new ProfileEditorWindow(draft, isNew: true);
        var saved = await win.ShowDialog<bool?>(this);
        if (saved != true || win.Result == null) return;
        ...
        InsertLog($"已新建方案：{win.Result.Name}");
        ReloadProfiles(win.Result.Id);
    }
    ...
    private void UpdateSeekNote(double? atSeconds = null)
    {
        ...
        if (!note.InRange) { TxtSeekNote.Text = $"{name} 超出音域"; return; }
        string mods = note.HasModifier ? note.Modifier.Label + "+" : "";
        string row = _profile.RowCount > 1 ? $"第{note.RowIndex + 1}行 " : "";
        string adj = note.Adjusted ? "（已调音）" : "";
        TxtSeekNote.Text = $"{name} {Music.DegreeName(note.Pitch)} · {row}{mods}{note.Key.Label}{adj}";
    }
    ...
    private async void ExportMidi_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            MidiExporter.Write(path, raw, "GameInstrumentPlayer 编辑");
    ...
    /// <summary>载入后自动挑最像主旋律的轨并选中。依据：非打击乐、轨名像旋律、音域贴合当前乐器方案。</summary>
    private void ChooseRecommendedTrack()
    {
        ...
        if (best == null)
        {
            InsertLog("没有适合当旋律的轨道（全是打击乐？），请手动点选一行。");
            return;
        }
    ...
    private double ScoreCandidate(TrackRowVM r)
    {
        ...
        if (notes.Count > 0)
        {
            var map = NoteMapper.Map(notes, 0, null, _profile);
    ...
    private MappingResult MapAt(int transpose) =>
        NoteMapper.Map(GetActiveRawNotes(), transpose, manualBasePitch: null, profile: _profile);
    ...
    private void ExportAhk_Click(object? sender, RoutedEventArgs e)
        => ExportSchedule(MacroExporter.Format.Autohotkey);
    ...
    private async void ExportSchedule(MacroExporter.Format format)
    {
        try
        {
            ...
            var timing = InputTiming.FromIndex(TimingCombo.SelectedIndex, _profile);

            string songName = _parsed == null ? "" : Path.GetFileNameWithoutExtension(_parsed.FilePath);
            string content = MacroExporter.Build(playable, format, speed, timing, songName, _profile);

            var (nCount, nEvents, seconds) = MacroExporter.Summarize(playable, speed, timing);
            string suggested = (string.IsNullOrWhiteSpace(songName) ? "instrument" : songName) +
            ...
            InsertLog($"已导出按键表：{playable.Count} 音 / {nEvents} 事件 / {seconds:F1}s" +
                      $"（方案 {_profile.Name}、速度 {speed * 100:F0}%、档位 {timing.Name}）");
            InsertLog($"　文件：{outPath}");
            switch (format)
            {
                case MacroExporter.Format.Autohotkey:
                    InsertLog("　AutoHotkey 用法：装好 AutoHotkey 后双击本文件；按 F8 开始，F9 退出。");
                    break;
                case MacroExporter.Format.LogitechGHub:
    ...
    private void BtnAutoTranspose_Click(object? sender, RoutedEventArgs e)
    {
        ...
        string sign = best > 0 ? "+" : "";
        if (bestSkip == 0)
            InsertLog($"一键移调：整体 {sign}{best} 半音后全部音在「{_profile.Name}」音域内。");
        else
            InsertLog($"一键移调：整体 {sign}{best} 半音后仍剩 {bestSkip} 个音超出音域（跨度过大，仍会空拍）");
        RefreshPreview();
    }
    ...
    private void Timing_Changed(object? sender, SelectionChangedEventArgs e)
    {
        ScheduleSave();
        if (!_uiReady) return;
        var t = InputTiming.FromIndex(TimingCombo.SelectedIndex, _profile);
        InsertLog($"输入兼容档位：{t.Name}（帧 {t.FrameMs:F0}ms、修饰键提前 {t.ModLeadMs:F0}ms、" +
                  $"重触发 {t.RetriggerMs:F0}ms、按住 {t.MinHoldMs:F0}ms）");
    }
    ...
    private void RefreshPreview()
    {
        ...
        if (m.InRangeCount == 0)
        {
            LblWarn.Text = m.Notes.Count == 0
                ? "该轨道/声道没有音符，请换一行。"
                : $"所有音都超出「{_profile.Name}」的音域（{_profile.RangeLabel}）。请用「一键移调」或换方案。";
            LblWarn.Foreground = warnColor;
        }
        else if (m.SkipCount > 0)
        {
            LblWarn.Text = $"有 {m.SkipCount} 个音超出「{_profile.Name}」的音域（{_profile.RangeLabel}），将自动空拍。" +
                           (m.AdjustedCount > 0 ? $"另有 {m.AdjustedCount} 个半音已按方案降级。" : "") +
                           "可用「一键移调」调整。";
            LblWarn.Foreground = warnColor;
        }
        else
        {
            LblWarn.Text = m.AdjustedCount > 0
                ? $"全部音可演奏；其中 {m.AdjustedCount} 个半音已按方案降到最近的音级。"
                : "全部音都在可演奏音域内，可直接演奏。";
    ...
    private void RequestPlay()
    {
        ...
        if (cd > 0)
        {
            _countdownLeft = cd;
            UpdateCountdownText();
            InsertLog($"{cd} 秒后开始——请切到游戏窗口并打开乐器…");
    ...
    private void UpdateCountdownText()
    {
        ...
        LblStatus.Text = $"{_countdownLeft} 秒后开始 —— 请切到游戏并打开乐器（F6 可取消）";
    ...
    private void StartPlayback()
    {
        ...
        engine.Timing = InputTiming.FromIndex(TimingCombo.SelectedIndex, _profile);
        engine.Play(_playNotes, speed, FixedLeadMs, loop, breath, _profile);
        ...
        InsertLog($"开始演奏（{_profile.Name}）；前台窗口：{(string.IsNullOrEmpty(fgTitle) ? "（读不到，可能未切到游戏）" : fgTitle)}");
        LblStatus.Foreground = OkBrush;
        LblStatus.FontSize = 22;
        ...
        try
        {
            ...
            try
            {
                using var s = Avalonia.Platform.AssetLoader.Open(
                    new Uri("avares://GameInstrumentPlayer/Assets/app.ico"));
