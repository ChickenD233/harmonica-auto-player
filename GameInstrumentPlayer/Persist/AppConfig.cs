...
namespace GameInstrumentPlayer.Persist;
...
public sealed class AppConfig
{
    ...
    public bool AutoMinimizeOnPlay { get; set; } = true;  // 演奏开始后自动最小化窗口
    public int TimingIndex { get; set; } = 1;         // 输入兼容档位：0稳健/1方案默认/2极限
    public string ProfileId { get; set; } = "";       // 上次用的乐器方案（空=用第一个）
    public string SkippedUpdateTag { get; set; } = "";   // 用户选择“跳过”的版本号（空=不跳过）

    private static string DirPath =>
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "GameInstrumentPlayer");
