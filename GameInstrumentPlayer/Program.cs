...
internal static class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        {
            try { GameInstrumentPlayer.Persist.LogFile.Append("[FATAL] " + (e.ExceptionObject?.ToString() ?? "未知异常")); }
            catch { }
        };

        if (!TryAcquireSingleInstance())
        {
            try { GameInstrumentPlayer.Persist.LogFile.Append("检测到已有一个实例在运行，本实例直接退出。"); }
            catch { }
    ...
    private static bool TryAcquireSingleInstance()
    {
        try
        {
            string dir = System.IO.Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "GameInstrumentPlayer");
