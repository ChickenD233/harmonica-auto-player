...
#
# 产物与线上 Release 一致：zip 根目录下只有 GameInstrumentPlayer.exe 与「更新说明.txt」。
...
rm -rf "$OUT"
# 体积相关参数（实测依据见《GameInstrumentPlayer-体积分析报告.md》，_sizecheck/ 保留原始日志）：
...
"$DOTNET" publish GameInstrumentPlayer.csproj -c Release \
...
if [ ! -f "$NOTES" ]; then
  ...
  NOTESTMP="$(mktemp)"
  printf 'MIDI 转游戏乐器演奏器\n\n（本次构建未附带更新说明）\n' > "$NOTESTMP"
  NOTES="$NOTESTMP"
fi

ZIP="release/GameInstrumentPlayer-win-x64.zip"
echo ">> 打包 $ZIP ..."
rm -f "$ZIP"
# 优先用 python3 打包：它能把中文文件名写成带 UTF-8 标志的条目，
# 避免 Windows 资源管理器解压出来是乱码（macOS 自带 zip 不会加这个标志）。
if command -v python3 >/dev/null 2>&1; then
  python3 - "$OUT/GameInstrumentPlayer.exe" "$NOTES" "$ZIP" <<'PY'
