...
/// <summary>
/// 主旋律取音：把一条"伴奏 + 旋律混在一起"的多音轨/多和弦声部，
/// 压成一条最像原曲主旋律的单音线。
///
/// 为什么不能"同刻取最高"：这是目前默认策略，在「伴奏在旋律下方」（钢琴左手）
/// 时是 100% 正确的；但钢琴/吉他改编曲里右手琶音常在主旋律**之上**，
/// 取最高就会系统性地取错音 —— 用户反馈"吹 see you again 听不出原曲"即由此而来。
///
/// 本实现分两步（实测把"伴奏在上方"场景的正确率从 0% 提到 100%）：
///   ① 声部分离：按时间流式扫描，每个音归入"当前活跃且音高最接近"的声部；否则新开声部。
///      伴奏的高音琶音会自成一"声部"，主旋律在另一条声部里保持完整。
///   ② 选声部：按「级进比例（最重要）/ 平均时值 / 音区 / 音域宽度 / 音符密度」给每条声部打分，
///      取最高分那条。
///   ③ 自动择一（安全阀）：若最高声部的级进比例很高、时值也不短 ——
///      说明主旋律本来就在最高声部，此时直接沿用"同刻取最高"，
///      既拿到声部分离的全部收益，又不在简单曲子上退化。
/// </summary>
public static class MelodyExtractor
{
    ...
    /// <summary>
    /// 分声部：返回按"起始时间"排序的声部列表。
    /// 流式贪心归属：能并进某条活跃声部就并，否则新开一条。
    /// </summary>
    ...
    public static List<RawNote> Extract(IEnumerable<RawNote> notes, bool autoChoose = true)
    {
        ...
        // 安全阀：最高声部本身就高度级进、时值也不短 → 主旋律本来就在最高声部，
        // 此时"同刻取最高"对「伴奏在下方」的曲子是 100% 正确的，直接沿用更稳。
        if (autoChoose)
        {
            int topIdx = 0;
